...
// Helper component for NFT price deltas
function NFTPriceDelta({ id, chain, address, notedAt, tweetTimestamp, sentiment }: { 
  id: string; 
  chain: string; 
  address: string; 
  notedAt: string; 
    ...
    (async () => {
      ...
      const fetchAtTime = async (offsetDays: number) => {
        ...
        if (offsetDays === 0 && tweetTimestamp) {
          return (await fetch(`/api/price/nft?chain=${chain}&address=${address}&timestamp=${tweetTimestamp}`)).json();
        } else {
...
export default function NFTPageIsland({ project, initialSignals }: Props) {
  ...
  useEffect(() => {
    ...
    for (const signal of signals) {
      ...
      if (signal.sentiment === 'bullish') entry.bullish++;
      else entry.bearish++;
    }
    ...
    const fetchPerformance = async () => {
      ...
      for (const [username, data] of userMap) {
        try {
          ...
          if (projectPerf) {
            const values = [];
            if (projectPerf.shortTerm !== null && projectPerf.shortTerm !== undefined) {
              values.push(projectPerf.shortTerm);
            }
            if (projectPerf.longTerm !== null && projectPerf.longTerm !== undefined) {
              values.push(projectPerf.longTerm);
            }
            if (values.length > 0) {
          ...
          leaderboardData.push({
            ...
            performance,
            shortTerm: projectPerf?.shortTerm ?? null,
            longTerm: projectPerf?.longTerm ?? null,
